...
class Image:
    ...
    @staticmethod
    def make_new(width, height, num_channels=4, clear_color=[0, 0, 0, 0]):
    ...
    def clear(self, color: typing.List):
        ...
        while j != end:
            ...
            for channel in range(0, len(color)):
                self.raw.image_data[j + channel] = color[channel]
...
if __name__ == '__main__':
    ...
    import sys
    if len(sys.argv) < 3:
        print('Usage: python {} <test_image> <expected_channels>'.format(sys.argv[0]))
        sys.exit(1)

    _test3(sys.argv[1], int(sys.argv[2]))
    # _test4()
