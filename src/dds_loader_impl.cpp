/*
    ...
    Vermilion Book - DDS File Support
        ...
        Adapted from information obtained at
...
//    { GL_COMPRESSED_RGB_S3TC_DXT3_EXT,  GL_NONE, GL_COMPRESSED_RGB_S3TC_DXT3_EXT, GL_ZERO, GL_ZERO, GL_ZERO,        GL_ZERO             },      // DDS_FORMAT_BC3_UNORM
    ...
    { GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT, GL_NONE, GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT, GL_RED, GL_GREEN, GL_BLUE,     GL_ONE          },      // DDS_FORMAT_BC6H_UF16
    { GL_COMPRESSED_RGB_BPTC_SIGNED_FLOAT, GL_NONE, GL_COMPRESSED_RGB_BPTC_SIGNED_FLOAT, GL_RED, GL_GREEN, GL_BLUE, GL_ONE         },   // DDS_FORMAT_BC6H_SF16
    { GL_NONE,              GL_NONE,            GL_NONE,            GL_ZERO,        GL_ZERO,        GL_ZERO,        GL_ZERO             },      // DDS_FORMAT_BC7_TYPELESS
    { GL_COMPRESSED_RGBA_BPTC_UNORM, GL_NONE, GL_COMPRESSED_RGBA_BPTC_UNORM, GL_RED, GL_GREEN, GL_BLUE, GL_ALPHA            },      // DDS_FORMAT_BC7_UNORM
    { GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM, GL_NONE, GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM, GL_RED, GL_GREEN, GL_BLUE, GL_ALPHA         }, // DDS_FORMAT_BC7_UNORM_SRGB
...
static bool vgl_DDSHeaderToImageDataHeader(const DDS_FILE_HEADER& header, dds::ImageData* image)
{
...
bool load_dds(const char *filename, ImageData *image, fo::Allocator *allocator) {
    FILE *f;
    bool ret = true;
    ...
    if (!f) {
        LOG_F(ERROR, "Failed to open file: %s", filename);
        return false;
    }
    ...
    if (file_header.magic != DDS_MAGIC) {
        LOG_F(ERROR, "Not a DDS file: %s", filename);
        fclose(f);
        return false;
    }
    ...
    if (!vgl_DDSHeaderToImageDataHeader(file_header, image)) {
        LOG_F(ERROR, "Failed to get translate DDS header to an ImageData header: %s", filename);
        ret = false;
    }
    ...
    if (image->target == GL_NONE) {
        LOG_F(ERROR, "Translated vgl header will have target GL_NONE: %s", filename);
        fclose(f);
        return false;
    }
    ...
    image->mip[0].data = image->allocator->allocate(image->totalDataSize);
