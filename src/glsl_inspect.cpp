...
InspectedGLSL::InspectedGLSL(GLuint handle, pmr::memory_resource &pmr)
    : mHandle(handle)
    , mPmrResource(&pmr) {
    ...
    if (mHandle > 0) {
        ...
        LOG_F(ERROR, "The program handle is invalid.");
    }
}
...
void InspectedGLSL::Print(fo::string_stream::Buffer &ss) const {
    ...
    ss << "General:"
       << "\n";
    ss << "num inputs = " << mInputs.size() << "\n";
    ss << "num outputs = " << mOutputs.size() << "\n";
    ss << "num uniform blocks = " << mUniformBlocks.size() << "\n";
    ss << "num atomic counter buffers = " << mAtomicCounterBuffers.size() << "\n";
    ss << "num uniforms = " << mUniforms.size() << "\n";
    ss << "num shader storage blocks = " << mShaderStorageBlocks.size() << "\n";
    ss << "num buffer variables = " << mBufferVariables.size() << "\n";
    ss << "\n";
    ...
    for (unsigned i = 0; i < mInputs.size(); ++i) {
        ...
        ss << "Input[" << i << "]:"
           << "\n";
        ss << "name = " << input.name << "\n";
        ss << "type = " << GetEnumName(input.type) << "\n";
        ss << "shader type = " << GetEnumShaderName(input.type) << "\n";
        ss << "location = " << input.location << "\n";
        ss << "array Size = " << input.arraySize << "\n";
        ss << "referenced by shaders = " << GetReferencedByShaderList(input.referencedBy) << "\n";
        ss << "is per patch = " << input.isPerPatch << "\n";
        ss << "location component = " << input.locationComponent << "\n";
        ss << "\n";
    }
    ...
    for (unsigned i = 0; i < mOutputs.size(); ++i) {
        ...
        ss << "Output[" << i << "]:"
           << "\n";
        ss << "name = " << output.name << "\n";
        ss << "type = " << GetEnumName(output.type) << "\n";
        ss << "shader type = " << GetEnumShaderName(output.type) << "\n";
        ss << "location = " << output.location << "\n";
        ss << "array Size = " << output.arraySize << "\n";
        ss << "referenced by shaders = " << GetReferencedByShaderList(output.referencedBy) << "\n";
        ss << "is per patch = " << output.isPerPatch << "\n";
        ss << "location component = " << output.locationIndex << "\n";
        ss << "location index = " << output.locationIndex << "\n";
        ss << "\n";
    }
    ...
    for (unsigned i = 0; i < mUniformBlocks.size(); ++i) {
        ...
        ss << "UniformBlock[" << i << "]:"
           << "\n";
        ss << "name = " << block.name << "\n";
        ss << "buffer binding = " << block.bufferBinding << "\n";
        ss << "buffer data size = " << block.bufferDataSize << "\n";
        ss << "referenced by shaders = " << GetReferencedByShaderList(block.referencedBy) << "\n";
        ss << "active variables = " << block.activeVariables.size() << "\n";

        // Format the uniform block and its data members to look like a declaration
        // in the shader code with helpful comments.
        ss << "declaration = "
           << "\n";
        ss << "  uniform " << block.name << "\n";
        ss << "  {"
           << "\n";
        for (unsigned v = 0; v < block.activeVariables.size(); ++v) {
            auto const &uniform = mUniforms[block.activeVariables[v]];

            ss << "      " << GetEnumShaderName(uniform.type) << " " << uniform.name;
            ...
            if (uniform.matrixStride > 0) {
                ss << " matrixStride=" << uniform.matrixStride;
                ss << " rowMajor=" << uniform.isRowMajor;
            }
            ss << "\n";
        }
        ss << "  };"
    ...
    for (unsigned i = 0; i < mAtomicCounterBuffers.size(); ++i) {
        ...
        ss << "AtomicCounterBuffer[" << i << "]:"
           << "\n";
        ss << "buffer binding = " << acBuffer.bufferBinding << "\n";
        ss << "buffer data size = " << acBuffer.bufferDataSize << "\n";
        ss << "referenced by shaders = " << GetReferencedByShaderList(acBuffer.referencedBy) << "\n";
        ss << "active variables = " << acBuffer.activeVariables.size() << "\n";
        ss << "\n";
    }
    ...
    for (unsigned i = 0; i < mUniforms.size(); ++i) {
        ...
        ss << "Uniform[" << i << "]:"
           << "\n";
        ss << "name = " << uniform.fullName << "\n";
        ss << "type = " << GetEnumName(uniform.type) << "\n";
        ss << "shader type = " << GetEnumShaderName(uniform.type) << "\n";
        ss << "referenced by shaders = " << GetReferencedByShaderList(uniform.referencedBy) << "\n";

        // In a uniform block.
        if (uniform.blockIndex >= 0) {
            ss << "array size = " << uniform.arraySize << "\n";
            ss << "offset = " << uniform.offset << "\n";
            ss << "uniform block index = " << uniform.blockIndex << "\n";
            ss << "array stride = " << uniform.arrayStride << "\n";
        ...
        // Atomic counter.
            ...
            ss << "atomic counter buffer index = " << uniform.atomicCounterBufferIndex << "\n";

            auto const &acBuffer = mAtomicCounterBuffers[uniform.atomicCounterBufferIndex];
            ss << "declaration = "
               << "\n";
            ss << "  layout(binding = " << acBuffer.bufferBinding << ", offset = " << uniform.offset;
            ss << ") uniform " << GetEnumShaderName(uniform.type) << " " << uniform.name << ";"
               << "\n";
        }
        ...
        for (unsigned v = 0; v < block.activeVariables.size(); ++v) {
            ...
            if (bufferVar.topLevelArrayStride != topLevelArrayStride) {
                if (topLevelArrayStride > 0) {
                    ...
                    ss << "      struct //";
                    ss << " offset=" << bufferVar.offset;
                    ss << " structSize=" << bufferVar.topLevelArrayStride;
                    ss << "\n";
                    ss << "      {"
                       << "\n";
                }
            ...
            ss << "      " << GetEnumShaderName(bufferVar.type) << " " << bufferVar.name;
            ...
            if (topLevelArrayStride > 0) {
                ss << " structOffset=" << (bufferVar.offset - topLevelArrayOffset);
            }
            ss << " bufferOffset=" << bufferVar.offset;
            if (bufferVar.arrayStride > 0) {
                ss << " arrayStride=" << bufferVar.arrayStride;
            }
            if (bufferVar.matrixStride > 0) {
                ss << " matrixStride=" << bufferVar.matrixStride;
                ss << " rowMajor=" << bufferVar.isRowMajor;
            }
            ss << "\n";
        }
        if (topLevelArrayStride > 0) {
    ...
    for (unsigned i = 0; i < mBufferVariables.size(); ++i) {
        ...
        ss << "BufferVariable[" << i << "]:"
           << "\n";
        ss << "name = " << bufferVar.fullName << "\n";
        ss << "type = " << GetEnumName(bufferVar.type) << "\n";
        ss << "shader type = " << GetEnumShaderName(bufferVar.type) << "\n";
        ss << "referenced by shaders = " << GetReferencedByShaderList(bufferVar.referencedBy) << "\n";

        // In a buffer block.
        if (bufferVar.blockIndex >= 0) {
            ss << "array size = " << bufferVar.arraySize << "\n";
            ss << "offset = " << bufferVar.offset << "\n";
            ss << "buffer block index = " << bufferVar.blockIndex << "\n";
            ss << "array stride = " << bufferVar.arrayStride << "\n";
            ss << "matrix stride = " << bufferVar.matrixStride << "\n";
        ...
        // Opaque type is not in a uniform block.
            ...
            ss << "declaration = "
               << "\n";
            ss << "  buffer " << GetEnumShaderName(bufferVar.type) << " " << bufferVar.name << ";"
               << "\n";
        }
...
void InspectedGLSL::ReflectProgramInputs() {
...
void InspectedGLSL::ReflectProgramOutputs() {
    GLint numResources = 0;
            ...
            if (info.arraySize > 1) {
                ...
                if (index != std::string::npos) {
                    ...
                    if (index2 != std::string::npos) {
                        ...
                        LOG_F(ERROR, "Only single dimensional arrays supported in GLSL uniforms.");
                    }
                    ...
                    LOG_F(ERROR, "Unexpected condition.");
                }
            }
...
void InspectedGLSL::ReflectAtomicCounterBuffers() {
    GLint numResources = 0;
...
void InspectedGLSL::ReflectSubroutines(GLenum programInterface, pmr::vector<std::string> &subroutines) {
    GLint numResources = 0;
    glGetProgramInterfaceiv(mHandle, programInterface, GL_ACTIVE_RESOURCES, &numResources);
    if (numResources > 0) {
        ...
        for (int i = 0; i < numResources; ++i) {
            GLint result = 0;
            glGetProgramResourceiv(mHandle, programInterface, i, 1, &nameLengthProperty, 1, nullptr, &result);
...
void InspectedGLSL::ReflectSubroutineUniforms(GLenum programInterface,
                                              pmr::vector<SubroutineUniform> &subUniforms) {
    GLint numResources = 0;
    glGetProgramInterfaceiv(mHandle, programInterface, GL_ACTIVE_RESOURCES, &numResources);
...
void InspectedGLSL::ReflectBufferVariables() {
    GLint numResources = 0;
...
void InspectedGLSL::ReflectTransformFeedbackVaryings() {
    GLint numResources = 0;
...
void InspectedGLSL::ReflectTransformFeedbackBuffers() {
    GLint numResources = 0;
...
std::string InspectedGLSL::GetEnumName(GLenum value) {
    for (int i = 0; 0 != msEnumMap[i].value; ++i) {
        auto const &item = msEnumMap[i];
        if (item.value == value) {
            return item.name;
...
std::string InspectedGLSL::GetEnumShaderName(GLenum value) {
    for (int i = 0; 0 != msEnumMap[i].value; ++i) {
        auto const &item = msEnumMap[i];
        if (item.value == value) {
...
std::string InspectedGLSL::GetReferencedByShaderList(GLint const referencedBy[6]) {
    std::string strList;

    if (referencedBy[ST_VERTEX])
        strList += "vertex ";
    if (referencedBy[ST_GEOMETRY])
        strList += "geometry ";
    if (referencedBy[ST_PIXEL])
        strList += "pixel ";
    if (referencedBy[ST_COMPUTE])
        strList += "compute ";
    if (referencedBy[ST_TESSCONTROL])
        strList += "tessControl ";
    if (referencedBy[ST_TESSEVALUATION])
        strList += "tessEvaluation ";

    return strList;
}

void InspectedGLSL::IntelWorkaround(std::string const &name, GLint results[]) {
...
#define ENUM(value, shadername, rows, cols, size)                                                            \
    { value, #value, #shadername, rows, cols, size }
const InspectedGLSL::EnumMap InspectedGLSL::msEnumMap[]{
