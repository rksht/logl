...
// xyspoon: Was initially static, but I'm exposing this function.
...
static nfcd_loc write(struct nfcd_ConfigData **cdp, int type, void *p, int count, int zeroes) {
    ...
    while (cd->used_bytes + total > cd->allocated_bytes) {
        ...
        cd = (struct nfcd_ConfigData *)cd->realloc(cd->realloc_user_data, cd, cd->total_bytes, new_total_bytes, __FILE__, __LINE__);
...
struct nfcd_ConfigData *nfcd_make(nfcd_realloc realloc, void *ud, int config_size, int stringtable_size) {
    ...
    struct nfcd_ConfigData *cd = (struct nfcd_ConfigData *)realloc(ud, NULL, 0, total_bytes, __FILE__, __LINE__);
...
nfcd_loc nfcd_root(struct nfcd_ConfigData *cd) { return cd->root; }

// Returns the type of the config data item `loc`.
int nfcd_type(struct nfcd_ConfigData *cd, nfcd_loc loc) { return LOC_TYPE(loc); }
...
nfcd_loc nfcd_object_keyloc(struct nfcd_ConfigData *cd, nfcd_loc object, int i) {
    struct nfcd_ObjectItem *item = nfcd_object_item(cd, object, i);
    if (!item)
        return nfcd_null();
    return item->key;
}
...
nfcd_loc nfcd_null() { return MAKE_LOC(NFCD_TYPE_NULL, 0); }

// Returns the `false` object.
nfcd_loc nfcd_false() { return MAKE_LOC(NFCD_TYPE_FALSE, 0); }

// Returns the `true` object.
nfcd_loc nfcd_true() { return MAKE_LOC(NFCD_TYPE_TRUE, 0); }
...
void nfcd_set_root(struct nfcd_ConfigData *cd, nfcd_loc loc) { cd->root = loc; }
