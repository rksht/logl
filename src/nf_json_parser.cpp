...
static void cb_grow(struct Parser *p, struct CharBuffer *cb)
{
	...
	cb->s = (char *)temp_realloc(p, cb->s, cb->allocated, cb->allocated*2);
	cb->allocated *= 2;
...
static void lb_grow(struct Parser *p, struct LocBuffer *lb)
{
	...
	lb->data = (nfcd_loc *)temp_realloc(p, lb->data, sizeof(nfcd_loc)*lb->allocated, sizeof(nfcd_loc)*lb->allocated*2);
	lb->allocated *= 2;
