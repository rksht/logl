...
void nfst_init(struct nfst_StringTable *st, int bytes, int average_strlen) {
    ...
    memset(hashtable_16(st),
           0,
           st->num_hash_slots * (st->uses_16_bit_hash_slots ? sizeof(uint16_t) : sizeof(uint32_t)));
...
// undefined behavior.
const char *nfst_to_string(struct nfst_StringTable *st, int symbol) { return strings(st) + symbol; }
...
static inline struct HashAndLength hash_and_length(const char *start) {
    ...
    struct HashAndLength result = { h, int(s - start) };
    return result;
}

static inline uint16_t *hashtable_16(struct nfst_StringTable *st) { return (uint16_t *)(st + 1); }

static inline uint32_t *hashtable_32(struct nfst_StringTable *st) { return (uint32_t *)(st + 1); }

static inline char *strings(struct nfst_StringTable *st) {
    return st->uses_16_bit_hash_slots ? (char *)(hashtable_16(st) + st->num_hash_slots)
