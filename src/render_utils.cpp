...
struct PascalsTriangle {
    ...
    PascalsTriangle(int n)
        : _row_length(n + 1)
        , _table(make_std_alloc<int>(eng::pmr_default_resource())) {

        _table.resize(_row_length * _row_length);
...
pmr::vector<f32> gaussian_kernel(int count) {
    ...
    pmr::vector<f32> vec(make_std_alloc<f32>(eng::pmr_default_resource()));
    vec.resize(count);
    std::copy(triangle.row_begin(count - 1), triangle.row_end(count - 1), vec.begin());
    const f32 sum = std::accumulate(vec.begin(), vec.end(), 0.0f);
    for (auto &coeff : vec) {
        coeff /= sum;
    }
    return vec;
