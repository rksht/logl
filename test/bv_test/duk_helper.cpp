...
bool CallDukMethod::CALL_OPERATOR(duk_context *ctx, int index_of_object, const char *method_name) {
    const int top_on_entry = duk_get_top(ctx);
...
WrappedDukContext::~WrappedDukContext() {
    if (_context) {
        if (_destroy_on_dtor) {
            ...
            ABORT_F("Dtor called on WrappedDukContext, but not destroying the duktape heap it owns");
        }
    }
}
