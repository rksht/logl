...
// Wrapper around a duktape context. Helps with... things.
struct WrappedDukContext : NonCopyable
{
    ...
    // Executes a code file. Will crash on error.
    void exec_file(const fs::path &path);

    // Push source from file onto the stack
    void push_source_file(const fs::path &path);
};
...
// Represents a qualified variable name. For use with `DukFnArg`.
    ...
    void push(duk_context *ctx) const
    {
        VT_SWITCH(_variant)
        {
            ...
            VT_CASE(_variant, DukObjectByName)
                :
            {
                get_qualified_duk_variable(
                  ctx, get_value<DukObjectByName>(_variant)._qualified_name);
            }
            break;
...
{
    // Returns C++ true if there was no errors, otherwise falls. The returned duktape value is on the top
    // of stack. Optionally you can choose to pop it.
    bool operator()(duk_context *ctx,
            const char *variable_name,
            const char *method_name,
            const std::vector<DukFnArg> &args);

    // For functions that don't return any value (aka return an undefined value), we gonna pop it if set
    // to true.
