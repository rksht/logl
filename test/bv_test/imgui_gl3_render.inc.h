...
#if 0
void imgl3_create_render_states()
{
	{
...
layout(binding = 0) uniform sampler2D sam_tex_imgl;
...
void imgl3_render_draw_data(ImDrawData *draw_data)
{
	...
	glBindBufferRange(GL_UNIFORM_BUFFER, 0, ubo_handle, 0, sizeof(eng::CameraTransformUB));

	glBufferSubData(
	  GL_UNIFORM_BUFFER, 0, sizeof(eng::CameraTransformUB), REINPCAST(void *, &camera_transform_ub));
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; ++cmd_i) {
			...
			if (p_cmd.UserCallback != nullptr) {
				...
				if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f &&
					...
					// Bind texture, Draw
					glBindSampler(0, 0);
					glBindTextureUnit(0, p_cmd.TextureId);
