...
#ifndef NORMAL_MAP_BINDING
#define NORMAL_MAP_BINDING 1
#endif

#ifndef BAR_MATERIAL_UBO_BINDING
#define BAR_MATERIAL_UBO_BINDING 2
#endif

#ifndef LIGHT_PROPERTIES_UBO_BINDING
#define LIGHT_PROPERTIES_UBO_BINDING 3
#endif
...
layout(binding = NORMAL_MAP_BINDING) uniform sampler2D normal_map;

// ------ Uniform blocks

layout(binding = BAR_MATERIAL_UBO_BINDING, std140) uniform BarMaterial {
    Material mat;
};
...
void main() {
    vec3 normal_sample = (texture(normal_map, fs_in.st).xyz) * 2 - 1.0;
    ...
    normal_sample = TBN * normalize(normal_sample);

    color = vec4(0.0, 0.0, 0.0, 1.0);

    for (uint i = 0; i < NUM_POINT_LIGHTS; i++) {
        Light l = light_properties.list[i];
        color.xyz += compute_point_light(l, mat, fs_in.position_world, normal_sample,
                                         mvp.eyepos_world.xyz - fs_in.position_world);
    }
}
