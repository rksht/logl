// Same thing as the other demo, but using ARB_vertex_attrib_binding
// facilities.
...
void read_tetinfos(fo::Array<TetInfo> &tets) {
    ...
    read_file("/home/snyp/gits/learnogl/test/instance_test/tetinfo.json", json_src);
...
template <> void init<App>(App &app) {
    log_info("Alignof(TetInfo) = %zu", alignof(TetInfo));
    log_info("Alignof(Vector3) = %zu, Sizeof(tet_verts) = %zu", alignof(Vector3), sizeof(tet_verts));
    ...
    // Shader
    {
        ...
        read_file("/home/snyp/gits/learnogl/test/instance_test/instanced_tets_rot.vert", vert_shader_src);
        read_file("/home/snyp/gits/learnogl/test/instance_test/instanced_tets.frag", frag_shader_src);
    ...
    // We will store the vertex positions in the first part of the vertex
    // buffer, and then a list of TetInfo structs for each tet (i.e {color,
    // scale, translate, orientation}.
    const size_t total_buffer_size =
        sizeof(tet_verts) + alignof(TetInfo) + size(app.tets) * sizeof(TetInfo);
    ...
    const size_t tetinfo_start =
        sizeof(tet_verts) % alignof(TetInfo) == 0
            ? sizeof(tet_verts)
            : (sizeof(tet_verts) + alignof(TetInfo) - sizeof(tet_verts) % alignof(TetInfo));

    glBufferSubData(GL_ARRAY_BUFFER, tetinfo_start, size(app.tets) * sizeof(TetInfo),
                    data(app.tets));
    ...
    constexpr struct {
        GLuint pos = 0;
        GLuint color = 1;
        GLuint scale_trans = 2;
        GLuint orientation = 3;
    } attr_loc; // Attribute locations for better readability

    constexpr struct {
        // 0 will be non instanced, we use it for position attribute
        GLuint non_instanced = 0;
        // 1 will be instanced, we use it for color, scale_trans, and orientation attributes
        GLuint instanced = 1;
    } binding_index; // We want the positions to be non-instanced, while other attributes per-instance

    // Specifying the vertex formats for each binding point. Note that the
    // stride should be explicitly set (as opposed to saying 0)
    glBindVertexBuffer(binding_index.non_instanced, app.tet_vbo, GLintptr(0), sizeof(Vector3));
    glBindVertexBuffer(binding_index.instanced, app.tet_vbo, tetinfo_start, sizeof(TetInfo));

    // Set up the formats for each attribute location, and also, link with the
    // appropriate binding index.

    glVertexAttribFormat(attr_loc.pos, 3, GL_FLOAT, GL_FALSE, 0);
    glVertexAttribBinding(attr_loc.pos, binding_index.non_instanced);

    glVertexAttribFormat(attr_loc.color, 3, GL_FLOAT, GL_FALSE, GLuint(offsetof(TetInfo, color)));
    glVertexAttribBinding(attr_loc.color, binding_index.instanced);

    glVertexAttribFormat(attr_loc.scale_trans, 4, GL_FLOAT, GL_FALSE, GLuint(offsetof(TetInfo, scale)));
    glVertexAttribBinding(attr_loc.scale_trans, binding_index.instanced);

    glVertexAttribFormat(attr_loc.orientation, 4, GL_FLOAT, GL_FALSE, GLuint(offsetof(TetInfo, orientation)));
    glVertexAttribBinding(attr_loc.orientation, binding_index.instanced);

    glEnableVertexAttribArray(attr_loc.pos);
    glEnableVertexAttribArray(attr_loc.color);
    glEnableVertexAttribArray(attr_loc.scale_trans);
    glEnableVertexAttribArray(attr_loc.orientation);

    // Specify the instance divisor for the instanced binding point
    glVertexBindingDivisor(binding_index.instanced, 1);

    debug("DONE setting up vertex formats");
    ...
    app.proj_mat = math::persp_proj(0.1, 100.0, one_deg_in_rad * 100.0, float(height) / width);
