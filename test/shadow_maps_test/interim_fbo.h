...
// An RGBA16F color attachment is what we will use to render the final scene into before post-processing on
// it.
struct FP16ColorFBO {
...
struct PostProcess_GaussianBlur {
    u8 kernel_size = 9;
    pmr::vector<float> weights = pmr::vector<float>(eng::make_std_alloc<float>(eng::pmr_default_resource()));
...
static void init_lerped_kernel(PostProcess_GaussianBlur &self) {
    // This calculates the weight and normalized offset for sampling in between two consecutive texels whose
    // usual gaussian weights would be the given arguments.
    ...
    // Shift the offsets so that they denote offset relative to the center sample
...
static void allocate_gl_resources(PostProcess_GaussianBlur &self) {
    // Create the blur param UBO
    eng::gl_desc::UniformBuffer gaussian_blur_ubo_desc(
        eng::create_uniform_buffer(sizeof(GaussianBlurConstantsUB), GL_DYNAMIC_DRAW),
        0,
        sizeof(GaussianBlurConstantsUB));

    eng::set_buffer_label(gaussian_blur_ubo_desc.handle(), "@blur_params_ub");

    self.ubo = gaussian_blur_ubo_desc.handle();
    self.ubo_bindpoint = eng::g_bs().bind_unique(gaussian_blur_ubo_desc);
    ...
    LOCAL_FUNC create_blur_program = [&]() {
        ...
        prog.fs = eng::create_shader_object(make_path(SOURCE_DIR, "data", "gaussian_blur_vsfs.glsl"),
                                            ...
                                            self.shader_defs);
        prog.program = eng::create_program(prog.vs, prog.fs);
...
void render_pp_gaussian_blur(PostProcess_GaussianBlur &self) {
    glUseProgram(self.current_program->program);
