...
class memory_resource {
  ...
  public:
    ...
    // `is_equal` is needed because polymorphic allocators are sometimes produced as a result of type erasure.
    // In that case, two different instances of a polymorphic_memory_resource may actually represent the same
    // underlying allocator and should compare equal, even though their addresses are different.
...
inline bool operator==(const memory_resource &a, const memory_resource &b) {
    // Call `is_equal` rather than using address comparisons because some polymorphic allocators are produced
    // as a result of type erasure.  In that case, `a` and `b` may contain `memory_resource`s with different
    // addresses which, nevertheless, should compare equal.
    return &a == &b || a.is_equal(b);
}

inline bool operator!=(const memory_resource &a, const memory_resource &b) { return !(a == b); }

namespace __details {

// STL allocator that holds a pointer to a polymorphic allocator resource. Used to implement
// `polymorphic_allocator`, which is a scoped allocator.
template <class Tp> class polymorphic_allocator_imp {
    memory_resource *m_resource;

  public:
    using value_type = Tp;

    // These types are old-fashioned, pre-C++11 requirements, still needed by g++'s `basic_string`
    // implementation.
    ...
    template <class U> polymorphic_allocator_imp(const polymorphic_allocator_imp<U> &other);
...
template <> struct aligned_chunk<16> {
    alignas(16) char x;
};
...
// this component.
template <class Allocator> class resource_adaptor_imp : public memory_resource {
    typename allocator_traits<Allocator>::template rebind_alloc<max_align_t> m_alloc;

    template <size_t Align> void *allocate_imp(size_t bytes);

    template <size_t Align> void deallocate_imp(void *p, size_t bytes);
  ...
  public:
    ...
    resource_adaptor_imp(Allocator2 &&a2,
                         typename enable_if<is_convertible<Allocator2, Allocator>::value, int>::type = 0);
...
using resource_adaptor =
    __details::resource_adaptor_imp<typename allocator_traits<Allocator>::template rebind_alloc<byte>>;
...
template <class Tp>
class polymorphic_allocator : public scoped_allocator_adaptor<__details::polymorphic_allocator_imp<Tp>> {
  ...
  public:
    ...
    template <typename U> struct rebind { typedef polymorphic_allocator<U> other; };
    ...
    memory_resource *resource() const { return this->outer_allocator().resource(); }
};
...
inline pmr::memory_resource *pmr::set_default_resource(pmr::memory_resource *r) {
    if (nullptr == r)
...
inline pmr::__details::resource_adaptor_imp<Allocator>::resource_adaptor_imp(
    Allocator2 &&a2, typename enable_if<is_convertible<Allocator2, Allocator>::value, int>::type)
    : m_alloc(forward<Allocator2>(a2)) {}

template <class Allocator>
template <size_t Align>
void *pmr::__details::resource_adaptor_imp<Allocator>::allocate_imp(size_t bytes) {
    typedef __details::aligned_chunk<Align> chunk;
    size_t chunks = (bytes + Align - 1) / Align;

    typedef typename allocator_traits<Allocator>::template rebind_traits<chunk> chunk_traits;
...
template <class Allocator>
template <size_t Align>
void pmr::__details::resource_adaptor_imp<Allocator>::deallocate_imp(void *p, size_t bytes) {
    typedef __details::aligned_chunk<Align> chunk;
    size_t chunks = (bytes + Align - 1) / Align;
...
template <class Allocator>
void *pmr::__details::resource_adaptor_imp<Allocator>::do_allocate(size_t bytes, size_t alignment) {
...
template <class Allocator>
void pmr::__details::resource_adaptor_imp<Allocator>::do_deallocate(void *p, size_t bytes, size_t alignment) {
    static const size_t max_natural_alignment = sizeof(max_align_t);
...
template <class Allocator>
bool pmr::__details::resource_adaptor_imp<Allocator>::do_is_equal(const memory_resource &other) const
    noexcept {
...
template <class Tp>
inline __pmrd::polymorphic_allocator_imp<Tp>::polymorphic_allocator_imp(pmr::memory_resource *r)
...
template <class Tp> inline Tp *__pmrd::polymorphic_allocator_imp<Tp>::allocate(size_t n) {
    return static_cast<Tp *>(m_resource->allocate(n * sizeof(Tp), alignof(Tp)));
}

template <class Tp> inline void __pmrd::polymorphic_allocator_imp<Tp>::deallocate(Tp *p, size_t n) {
...
template <class Tp> inline pmr::memory_resource *__pmrd::polymorphic_allocator_imp<Tp>::resource() const {
    return m_resource;
}

template <class T1, class T2>
inline bool __pmrd::operator==(const __pmrd::polymorphic_allocator_imp<T1> &a,
                               const __pmrd::polymorphic_allocator_imp<T2> &b) {
    // `operator==` for `memory_resource` first checks for equality of addresses and calls `is_equal` only if
    // the addresses differ.  The call `is_equal` because some polymorphic allocators are produced as a result
    // of type erasure.  In that case, `a` and `b` may contain `memory_resource`s with different addresses
    // which, nevertheless, should compare equal.
